...
- **SHOULD** use `/clear` between unrelated tasks to maintain a clean context window.
- **SHOULD** keep CLAUDE.md concise. Only include universally applicable rules. Remove anything Claude already does correctly without instruction.
- **SHOULD** prefer pointers to files (`see src/auth/README.md`) over pasting code snippets that go stale.
- **SHOULD** document what Claude gets wrong, not what it already does right.
- **SHOULD** customize compaction behavior: "When compacting, always preserve the full list of modified files and any test commands."
- **SHOULD** delegate research to subagents to preserve main context for implementation.